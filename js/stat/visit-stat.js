...
class Pagination {
  ...
  initPaginationBar() { // pagination 컴포넌트 초기화
  ...
  initMonthPaginationBar() { // pagination 컴포넌트 초기화
    const paginationDiv = document.querySelector(".inner div#pagination")
    paginationDiv.innerHTML = ""
    if(this.elNum == 0) {
      const listDiv = document.querySelector("table#list-table")
  ...
  async showCurrentPageItems() {
    ...
    tableDiv.innerHTML = `      <table id="list-table" class="day-list">
        <tr id="table-key">
          <th>회원번호</th>
          <th>이름</th>
          <th>수강그룹</th>
...
async function getMonthQueries(value) {
  ...
  const querySnapShot = await getDocs(q)
  // 쿼리한걸로 user_id를 키값으로 dictionary 생성하자
  querySnapShot.forEach((doc) => {
    const id = doc.data().user_id
  ...
  showMonthList(reducedMonthList)
...
function reduceMonthList(attendances) {
  const reducedAttendaces = attendances.reduce((acc, attend) => {
    ...
    if(curMember == undefined) { // 처음 등장한 사람이면
      acc.push({user_id : attend.user_id, name : "이름", phoneNum : "010-0000-0000", dates : [attend.attend_date]})
      // 허허 이거 reduce콜백 안이어서 await 못하네 이런
    } else { // 이미 있던 사람이면
      curMember.dates.push(attend.attend_date)
    }
    return acc  
  }, [])
